...
const fetchProject = async(id) => {
  try {
    ...
    const response = await axios.get(`http://localhost:6001/fetch-project/${id}`, config);
...
const handleApproveSubmission = async() => {
  try {
    ...
    const response = await axios.get(`http://localhost:6001/approve-submission/${params['id']}`, config);
    fetchProject(params['id']);
...
const fetchChats = async() => {
  try {
    const token = localStorage.getItem("token");
    const config = { headers: { Authorization: `Bearer ${token}` } };
    const response = await axios.get(`http://localhost:6001/fetch-chats/${params['id']}`, config);
